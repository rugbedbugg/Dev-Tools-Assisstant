class DeveloperToolsPrompts:
    ...
    TOOL_EXTRACTION_SYSTEM = """You are a tech researcher. Extract specific tool, library, platform or service names from articles . Focus on actual products/tools that developers can use, not general concepts or features"""
    ...
    Pay special attention to programming languages, frameworks, APIs, SDKs, and developement tools
    """

    @staticmethod
    def tool_analysis_user(query: str, company_data: str) -> str:
        return f"""Company/Tool: {query}
        Website Content: {company_data[:2500]}
        
        Analyze this content from a developer's perspective and provide:
        - pricing_model: One of "Free", "Freemium", "Paid" or "Unknown"
        - is_open_source: True if open source, false if proprietary, null if unclear
        - tech_stack: List of programming languages, frameworks, databases, APIs or technologies
        - description: Brief 1-sentence description focusing on what this tool does for developers
        - api_available: True if REST API, GraphQL, SDL or programmatic access is mentioned
        - language_support: List of supported programming languages mentioned explicitly (e.g. Python, Java)
        - integration_capabilities: List of tools/platforms it integrates with (e.g. GitHub, VS Code)
        
        Focus on developer-relevant features like APIs, SDKs, language support, integrations and
"""

    # Recommendation prompts
    RECOMMENDATIONS_SYSTEM = """You are a senior software providing quick, concise tech recommendations.
    ...
    def recommendations_user(query: str, company_data: str) -> str:
        ...
        Tools/Technologies: {company_data}
